...
const generateQuestions=async()=>{
  ...
  try{
    const text=await callClaude([{role:"user",content:`You are Ava, an expert AI interview coach conducting a REAL mock interview. Generate exactly 7 interview questions. Return ONLY a JSON array of 7 strings, no other text.
...
CRITICAL QUESTION ORDER — follow this exactly like a real interview:
1. "Tell me about yourself" — warm opener, let them set the stage (ALWAYS first)
2. "Why are you interested in this role/company?" — motivation & fit
3. Behavioral question using STAR method — past experience
4. Situational/competency question — how they handle scenarios
5. Role-specific technical or domain knowledge question
6. Culture fit / values / working style question
7. "Do you have any questions for us?" style closer OR a strengths-based final question

Rules:
- Question 1 MUST be a "tell me about yourself" variant
- Questions must feel natural, warm, and like a real ${config.seniority} ${config.role} interview
- Be specific to ${config.industry} context
- No quizzing — these are conversation starters`}]);
    const parsed=parseJSON(text);
    if(Array.isArray(parsed)&&parsed.length>0){
      setQuestions(parsed);
      setAvaMessage(`I've prepared ${parsed.length} questions for your ${config.role} interview. Let's begin!`);
    } else throw new Error();
  }catch{
    setQuestions([
      `Let's start with the classic opener — tell me about yourself and your journey to becoming a ${config.role}.`,
      `What specifically drew you to apply for this ${config.role} position, and why now?`,
      `Tell me about a time you faced a significant challenge at work. What was the situation and how did you handle it?`,
      `Imagine you've just joined our team as a ${config.seniority} ${config.role}. In your first 90 days, what would you prioritize and why?`,
      `What would you say is your strongest skill relevant to this ${config.role} role, and can you give me a concrete example of it in action?`,
      `How do you prefer to work — independently or collaboratively — and how do you adapt when the environment is different from your preference?`,
      `That's all the questions I have for you today. Is there anything about the role, team, or company you'd like to know more about?`,
    ]);
    setAvaMessage("I've prepared 7 questions for your mock interview. Let's get started!");
  }
  ...
  useEffect(()=>{
    ...
    if(qIdx===0){
  const intro=`Hi ${user.name.split(" ")[0]}, welcome to your mock interview for the ${config.role} role! I'm Ava. I'll be your interviewer today — I'll ask you ${questions.length} questions just like a real interview. Take your time, speak naturally, and don't worry about being perfect — that's what practice is for. After each answer I'll give you feedback. Ready? Let's begin. ${currentQ}`;
  setAvaMessage(`Welcome ${user.name.split(" ")[0]}! This is your ${config.role} mock interview — ${questions.length} questions, real interview format.`);
  speakText(intro, startListening);
    } else {
const bridges=[
  `Great, thank you. Let's move on. ${currentQ}`,
  `Good. Next question. ${currentQ}`,
  `Thank you for that. ${currentQ}`,
  `Noted, thank you. ${currentQ}`,
  `Last question. ${currentQ}`,
];
